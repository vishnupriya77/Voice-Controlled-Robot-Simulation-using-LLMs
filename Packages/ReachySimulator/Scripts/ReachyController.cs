    ...
    public class ReachyController : MonoBehaviour
    {
        ...
        public void HandleCommand(Dictionary<JointId, float> commands)
        {
            ...
            foreach (KeyValuePair<JointId, float> kvp in commands)
            {
                ...
                if (motorName == "neck_roll") { containNeckCommand = true; headOrientation[0] = kvp.Value; }
                if (motorName == "neck_pitch") { containNeckCommand = true; headOrientation[1] = kvp.Value; }
                if (motorName == "neck_yaw") { containNeckCommand = true; headOrientation[2] = -kvp.Value; }
            }

            if (containNeckCommand)
            {
                UnityEngine.Quaternion euler_request =
                    UnityEngine.Quaternion.Euler(Vector3.forward * headOrientation[2]) *
                    UnityEngine.Quaternion.Euler(Vector3.up * -headOrientation[0]) *
                    UnityEngine.Quaternion.Euler(Vector3.right * headOrientation[1]);
        ...
        public List<SerializableMotor> GetCurrentMotorsState(Dictionary<JointId, JointField> request)
        {
            ...
            foreach (KeyValuePair<JointId, JointField> kvp in request)
            {
                ...
                position -= m.offset;
                target_position -= m.offset;

                motorsList.Add(new SerializableMotor
                {
                    name = m.name,
                    uid = m.uid,
                    present_position = Mathf.Deg2Rad * position,
                    goal_position = Mathf.Deg2Rad * target_position,
                    isCompliant = compliancy
                });
            }
        ...
        public List<SerializableSensor> GetCurrentSensorsState(Google.Protobuf.Collections.RepeatedField<Reachy.Sdk.Joint.SensorId> request)
        {
            ...
            {
                Sensor s = sensor.IdCase switch
                {
                    SensorId.IdOneofCase.Name => name2sensor[sensor.Name],
                    SensorId.IdOneofCase.Uid => sensors[sensor.Uid],
                    _ => null
                };

                if (s != null)
                {
                    sensorsList.Add(new SerializableSensor
                    {
                        name = s.name,
                        sensor_state = s.currentState
                    });
                }
            }
        ...
        public List<SerializableFan> GetCurrentFansState(Google.Protobuf.Collections.RepeatedField<Reachy.Sdk.Fan.FanId> request)
        {
            ...
            {
                Fan f = fan.IdCase switch
                {
                    FanId.IdOneofCase.Name => name2fan[fan.Name],
                    FanId.IdOneofCase.Uid => fans[fan.Uid],
                    _ => null
                };

                if (f != null)
                {
                    fansList.Add(new SerializableFan
                    {
                        name = f.name,
                        fan_state = f.state
                    });
...
// Restore method: HandleCameraService (dummy safe version for now)
public void HandleCameraZoom(CameraId cameraId, ZoomLevelPossibilities level)
{
    Debug.Log($"HandleCameraZoom called for {cameraId} with level {level}");
    // You can implement zoom logic later
}

public ZoomLevel GetCameraZoom(CameraId cameraId)
{
    Debug.Log($"GetCameraZoom called for {cameraId}, returning default zoom level");
    return new ZoomLevel { Level = ZoomLevelPossibilities.Out }; // Dummy response
}
    }
}